...
setwd("/home/barefootbushman/Desktop/DWER Thresholds analysis/DWER_Thresholds/EMP_78")

Y <- comm$Dampland_78
wat.obs <- data.ls$'61613231'
ahd <- AHD.data.list$EMP78.AHD
ahd.p <- Params.data.list$EMP78.params
WL_group <- "ground"
gw.plot <- Site.GW.plots$emp78.p
a.ele <- 67.988
b.ele <- 68.362
c.ele <- 69.038
d.ele <- 69.989
...
Y <- Y[ Y$year != 1997, , drop=TRUE] # No water level data for 1997
Y <- Y[ Y$year != 1998, , drop=TRUE] # No water level data for 1998
Y2 <- comm_prep(Y)
studyDesign <- Y2[[2]]
