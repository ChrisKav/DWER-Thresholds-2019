...
setwd("/home/barefootbushman/Desktop/DWER Thresholds analysis/DWER_Thresholds/Wilgarup")

Y <- comm$Wilgarup
wat.obs <- data.ls$'61618500'
ahd <- AHD.data.list$Wilgarup.AHD
ahd.p <- Params.data.list$Wilgarup.params
WL_group <- "ground"
gw.plot <- Site.GW.plots$wilg.p
a.ele <- 6.674
b.ele <- 6.949
c.ele <- 7.16
d.ele <- 7.087
...
# 1. Build HMSC object
Y <- Y[ Y$plot != "Z", , drop=TRUE] # No plot elevation for plot Z
Y2 <- comm_prep(Y)
studyDesign <- Y2[[2]]
...
GW.levs.d$Plot <- "D"
GW.levs.x <- GW.levs
GW.levs.x$meanGW <- GW.levs.x$meanGW - x.ele
GW.levs.x$Plot <- "X"
GW.levs.y <- GW.levs
GW.levs.y$meanGW <- GW.levs.y$meanGW - y.ele
GW.levs.y$Plot <- "Y"
GW.levs <- rbind(GW.levs.a,GW.levs.b,GW.levs.c,GW.levs.d,GW.levs.x,GW.levs.y)
